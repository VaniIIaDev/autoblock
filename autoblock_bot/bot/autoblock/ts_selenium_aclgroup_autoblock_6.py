...
user_block_note = "부적절한 계정명 자동 차단(봇 오작동 시 사용자:Vanilla/긴급정지 문서에 토론 남겨주세요)"
    ...
    if document:
        ...
        if username and document not in blocked_users[username]:
            if document.startswith("사용자:"):
                block_user(username, user_block_note)
            else:
                block_user(username, note)
